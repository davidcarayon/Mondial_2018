...
pourcentage_table$pays<-factor(pourcentage_table$pays,levels=c("France","non renseigné","Belgique"))
##################################
pourcentage_table %>% 
  ggplot(aes(x=1,y=n*100,fill = pays))+
  geom_bar(stat="identity",position = position_fill(reverse = TRUE),width = 1) +
 geom_text(aes(x = 1, y =cumsum(n[c(2,3,1)])- (0.5 * n[c(2,3,1)]),
                                             label = paste0(c("France","non renseigné","Belgique"),"\n",round(n[c(2,3,1)]*100,0),"%")), size=10,angle=90)+
  coord_flip()+
  scale_fill_manual(values=c("France"="blue","non renseigné"="white","Belgique"="red"))+
  theme_void()+

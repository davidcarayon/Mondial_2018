...
<!-- ### Probabilité estimée pour le match de ce soir -->

<!-- ```{r graph pronostic,echo=FALSE,warning=FALSE,message=FALSE, fig.align = "center",fig.width=12,fig.height=3} -->
<!-- table_pronostics %>%  -->
<!--   filter(id_match=="M62-DEMI-2") %>%  -->
<!--   group_by(pays) %>%  -->
<!--   summarise(n=n()) %>%  -->
<!--   mutate(n=n/sum(n)) %>%  -->
<!--   as.data.frame()->pourcentage_table -->
<!--  ########################################### -->
<!-- pourcentage_table$pays[is.na(pourcentage_table$pays)]<-"non renseigné" -->
<!-- pourcentage_table$pays<-factor(pourcentage_table$pays,levels=c("Angleterre","non renseigné","Croatie")) -->
<!-- ################################## -->
<!-- pourcentage_table %>%  -->
<!--   ggplot(aes(x=1,y=n*100,fill = pays))+ -->
<!--   geom_bar(stat="identity",position = position_fill(reverse = TRUE),width = 1) + -->
<!--  geom_text(aes(x = 1, y =cumsum(n[c(1,3,2)])- (0.5 * n[c(1,3,2)]), -->
<!--                                              label = paste0(c("Angleterre","non renseigné","Croatie"),"\n",round(n[c(1,3,2)]*100,0),"%")), size=10,angle=90)+ -->
<!--   coord_flip()+ -->
<!--   scale_fill_manual(values=c("Angleterre"="lightblue","non renseigné"="white","Croatie"="lightsalmon1"))+ -->
<!--   theme_void()+ -->
...
### Pronostiqueurs toujours en course pour le BONUS Champion

Tout au long du tournoi, vous pouvez suivre le nombre de pronostiqueurs ayant encore leurs choix d'équipe championne du monde toujours en course pour le **BONUS**.
<!-- `r anicon::nia(text="BONUS",animate="tada",color="red",fixed_width=TRUE)`. -->

```{r,graph funnel,echo=FALSE,warning=FALSE,message=FALSE, fig.align = "center"}

`%not_in%` = Negate(`%in%`) 
################################
funnel_fonction<-function(tour){
  bind_rows(
    table_evenements %>% 
      filter(id_tour==tour) %>% 
      dplyr::select(Equipe.A) %>% 
      rename("Equipe"="Equipe.A"),
    table_evenements %>% 
      filter(id_tour==tour) %>% 
      dplyr::select(Equipe.B) %>% 
      rename("Equipe"="Equipe.B")
  ) %>% dplyr::select(Equipe) %>% pull()->equipes
  table_participants %>% 
    dplyr::filter(champion %in% equipes) %>%
    nrow() ->nbr
  table_participants %>% 
    dplyr::filter(champion %not_in% equipes) %>% 
    dplyr::select(champion) %>% distinct() %>%  pull()->equipes_sortantes
  return(list(nbr,equipes_sortantes))
  # return(nbr)
}
##########################################
nbr_huit<-nbr_QUART<-nbr_DEMI<-nbr_FINAL<-NA
nbr_huit<-funnel_fonction(tour="HUIT")[[1]]
nbr_QUART<-funnel_fonction(tour="QUART")[[1]]
nbr_DEMI<-funnel_fonction(tour="DEMI")[[1]]
nbr_FINAL<-funnel_fonction(tour="FINAL")[[1]]
#####################################################
if(is.na(nbr_QUART)){
  nbr_QUART<-nbr_DEMI<-nbr_FINAL<-nbr_huit
  stage = c("Poules", "Huitièmes")
  value = c(44, nbr_huit)
  
}else{
  if(is.na(nbr_DEMI)){
    stage = c("Poules", "Huitièmes","Quarts")
    value = c(44, nbr_huit,nbr_QUART)
    nbr_DEMI<-nbr_FINAL<-nbr_QUART
  }else{
    if(is.na(nbr_FINAL)){
      stage = c("Poules", "Huitièmes","Quarts","Demi")
      value = c(44, nbr_huit,nbr_QUART,nbr_DEMI)
      nbr_FINAL<-nbr_DEMI
    }else{
      stage = c("Poules", "Huitièmes","Quarts","Demi","Finale")
      value = c(44, nbr_huit,nbr_QUART,nbr_DEMI,nbr_FINAL)
      # stage = c("Poules", "Huitièmes","Quarts","Demi"
    }
  }
}
####################################################
funnel <- data.frame(stage , value )
names(funnel)<-c("description","value")
########################################################
# funnel %>% 
#   e_charts() %>% 
#   e_funnel(value, description) %>% 
#   e_title("Nombre de pronostiqueurs ayant leur 'champion' encore en jeu") %>% 
#   e_legend(
#     # selectedMode = "single",
#     left = "right",
...
amFunnelChart(dataProvider = funnel, startX = -500,
              depth3D = 50, angle = 60, 
              outlineColor = '#FFFFFF',
              outLineThickness =5, 
              ...
              marginRight = 300, startDuration = 5, theme = "light",
              rotate=180) %>>%
  amOptions( main = "Nombre de participants ayant leur champion encore en jeu",
             mainColor = "#68838B", mainSize = 20, creditsPosition = "top-right") %>>% 
  setLegend(position = "bottom", useMarkerColorForLabels = TRUE) %>>%
  plot() 
```
Les pays qui sont rentrés à la maison :

<!-- `r anicon::nia(text=paste(funnel_fonction(tour="HUIT")[[2]],collapse=", "),animate="passing",speed="slow",size=2)`   -->
<!-- * `r  anicon::nia(text=paste(funnel_fonction(tour="QUART")[[2]],collapse=", "),animate="passing",speed="slow",size=2)` -->
<!-- * `r anicon::nia(text=paste(funnel_fonction(tour="DEMI")[[2]],collapse=", "),animate="passing",speed="slow",size=2)` -->
* `r anicon::nia(text=paste(funnel_fonction(tour="FINAL")[[2]],collapse=", "),animate="passing",speed="slow",size=1)`
...
<!-- ![reus](https://media0.giphy.com/media/juddKSbDBJgRy/giphy.gif?cid=e1bb72ff5b4708c46d3375414de312e2)   -->
 ![pavard](https://media0.giphy.com/media/2mDmdNdJ9oD3ibyMxb/giphy.gif?cid=e1bb72ff5b47088051654c4e63ccf715)  


<!-- ![ngolo+kante](https://media1.giphy.com/media/FsV6VaRz2OQOnyjfF4/giphy.gif?cid=e1bb72ff5b460dae744a734c4119966f)   -->
<!-- ![Alisson+Becker](https://media3.giphy.com/media/xUOxfoAs1aZNja273W/giphy.gif?cid=e1bb72ff5b43103c703044626bdc3c68)   -->
<!-- ![neymar+](https://media0.giphy.com/media/sDUtsdsrEuad2/giphy.gif?cid=e1bb72ff5b3c7881347843392ea60df3)   -->
<!-- ![dembele+ousmane+](https://media2.giphy.com/media/qsWWHpKvsGDEA/giphy-downsized-medium.gif?cid=e1bb72ff5b3b2a20585646772ee22604)   -->
<!-- ![khedira](https://media1.giphy.com/media/7sPmucMC8zHry/giphy-downsized.gif?cid=e1bb72ff5b33548e6c674b6f49c0b31d)   -->
<!-- ![mendy](https://media2.giphy.com/media/EbGMxSL0ZuRTW/giphy-downsized.gif?cid=e1bb72ff5b348ac951454b2e32958df4)   -->
<!-- ![thiago+silva](https://media2.giphy.com/media/SKT3N37H9KtFPdCKkz/giphy-downsized.gif?cid=e1bb72ff5b35e0c2574538795100d784)   -->
<!-- ![Busquets+spain](https://media0.giphy.com/media/OvJQI1Ub7twk0/giphy.gif?cid=e1bb72ff5b2ca00476494a542e758f8b)   -->
<!-- ![Lukaku](https://media0.giphy.com/media/8maPpCVRIpFyTs6FMp/giphy.gif?cid=e1bb72ff5b2b4e0d313670356fb074fa)   -->
...
ggplotly(p2, height =900, width =750) %>%
  animation_opts(frame = 200*length(unique(pronostics$datetime)),
                 easing = "linear",
                 redraw = FALSE)
```
